...
```{r}
batch_date <- "2022-08-17"
# ^ this line is updated by code at the end of 1_...Rmd
```
...
#remove people who requested to unsubscribe but didn't do it through the tool
emails <- emails[!(emails$email %in% c("")),]
...
for (i in 1:nrow(emails)){
  ...
  if(file.exists(emails$newsletter_file_location[i])){
    ...
    identity_check <- readLines(emails$newsletter_file_location[i],encoding = "UTF-8") %>% 
      paste0(collapse="") %>% 
      gsub("’","'",.) %>% #circumvent encoding issue
      grepl(emails$name[i],.)
