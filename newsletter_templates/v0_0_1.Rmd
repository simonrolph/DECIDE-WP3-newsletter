...
if (!is.na(params$irecord_username)){
  ...
  irecord_records <- get_records_irecord(params$irecord_username,nrecords = 10,key)
  irecord_records$website <- "iRecord"
  
  all_records <- rbind(all_records,irecord_records)
}

#get records from iSpot
if (!is.na(params$ispot_username)){
  key <- readLines(file("../.secrets/ispot_key.txt",open="r"))
  ispot_records <- get_records_ispot(params$ispot_username,nrecords = 10,key)
  ispot_records$website <- "iSpot"
  
  all_records <- rbind(all_records,ispot_records)
}
...
if (!is.na(params$inat_username)){
  ...
  all_records <- rbind(all_records,inat_records)
}
