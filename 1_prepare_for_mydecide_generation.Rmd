...
```{r}
#get the date of that the most recent personal data was downloaded from the sign-up app
date_of_most_recent_user_data <- list.files("data_personal/download_from_signup_app") %>% gsub("data-","",.) %>% gsub(".csv","",.) %>% as.Date() %>% max()

#the 30 day period
start_date <- date_of_most_recent_user_data-32
end_date <- date_of_most_recent_user_data-1


#load dataframe
user_db <- read_csv(paste0("data_personal/download_from_signup_app/data-",date_of_most_recent_user_data,".csv")) 
names(user_db)[1] <- "user_id"

saveRDS(user_db,paste0("data_personal/data_export/users_",date_of_most_recent_user_data,".RDS"))
...
```{r}
#note that the top-up is less needed as last time it was run the users had a total of >68k records but only 124 were missed
...
#automatically assign new tracks (unfinished)
if(F){
  last_track <- user_tracks$group %>% last() %>% gsub("t","",.) %>% as.numeric() #last track
  
  #find users without tracks
  new_emails <- user_db %>% filter(!(email %in% user_tracks$email)) %>% pull(email)
  new_emails <- new_emails[!grepl("zzz",new_emails)]
  
  #plenty of new tracks # TODO
  new_tracks <- rep(paste0("t",1:18),100)[(last_track+1):100]
  length(new_tracks) <- length(new_emails)
...
saveRDS(email_log,paste0("data_personal/data_export/email_log_",Sys.Date(),".RDS"))
...
#Update `.sbatch` file to have the correct job array length
sbatchfile <- readLines("generate_mydecide.sbatch")
sbatchfile[8] <- paste0("#SBATCH --array=1-", length(markdown_params_list))
writeLines(sbatchfile,"generate_mydecide.sbatch")

#update lines in 3_... file to reflect the files paths to the latest batch of newsletters 
send_script <- readLines("3_send_from_rshiny_connect.Rmd")
send_script[6] <- paste0("  - \"newsletters/",end_date,"/\"")
send_script[7] <- paste0("  - \"data_personal/emails_",end_date,".rds\"")
send_script[14] <- paste0('batch_date <- "',end_date,'"')
writeLines(send_script,"3_send_from_rshiny_connect.Rmd")


```

## Next processing stage - generating email contant

Commit to github

The items needed for generating emails on JASMIN are:

 * The `records_{DATE}.RDS` object
 * The parameters for the `markdown_params_list` saved as `data_personal/markdown_params_list.rds`.
 
Log into JASMIN with 

`ssh -A simrol@sci1.jasmin.ac.uk`

And navigate to directory with

`cd DECIDE/DECIDE-WP3-newsletter`
...
`sbatch generate_mydecide_test.sbatch`

Check the job progress with 

`squeue -u simrol`

Can delete any rendered test emails using a line like this (specifing the folder by replacing the XX)

`rm -r newsletters/2022-XX-XX`

Then if all working correctly then run the main script:

`sbatch generate_mydecide.sbatch`
